            ...
            foreach ($this->registry->getDefinitions() as $definitions) {
                foreach ($definitions as $definition) {
                    $records[] = [
                        ...
                        'manifest' => $this->registry->getManifestBasename($definition['package']),
        ...
        if ($kind === null || $kind === 'event') {
            ...
            foreach ($events as $event) {
                $records[] = [
                    ...
                    'manifest' => $this->registry->getManifestBasename($event['package']),
                    'source_file' => null,
                    'event' => $event['name'],
                    'priority' => null,
                    'contexts' => [],
        ...
        if ($kind === null || $kind === 'listener') {
            foreach ($this->registry->getAllListeners() as $listener) {
                $records[] = [
                    ...
                    'package' => $listener['package'],
                    'manifest' => $this->registry->getManifestBasename($listener['package']),
